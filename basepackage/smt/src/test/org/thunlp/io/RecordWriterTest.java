...
public class RecordWriterTest extends TestCase {
	...
	public void testWritePlainText() throws IOException {
		RecordWriter writer = new RecordWriter(filename, "UTF-8", RecordWriter.TYPE_PLAIN_TEXT, false);
	...
	public void testWriteGzippedText() throws IOException {
		RecordWriter writer = new RecordWriter(filename + ".gz", "UTF-8", RecordWriter.TYPE_GZIPPED_TEXT, false);
		for (int i = 0; i < values.length; i++) {
			writer.add(values[i]);
		}
		writer.close();
		...
		BufferedReader reader = new BufferedReader(
				new InputStreamReader(new GZIPInputStream(new FileInputStream(filename + ".gz")), "UTF-8"));
		int n = 0;
		String line;
		while ((line = reader.readLine()) != null) {
			Assert.assertEquals(values[n], line);
	...
	public void testWriteZipText() throws IOException {
		RecordWriter writer = new RecordWriter(filename + ".zip", "UTF-8", RecordWriter.TYPE_ZIPPED_TEXT, false);
		for (int i = 0; i < values.length; i++) {
			writer.add(values[i]);
		}
		writer.close();
		...
		// Check the result.
		ZipInputStream zin = new ZipInputStream(new FileInputStream(filename + ".zip"));
	...
	public void testWriteSequenceFile() throws IOException {
		RecordWriter writer = new RecordWriter(filename + ".sf", "UTF-8", RecordWriter.TYPE_SEQUENCE_FILE, false);
		for (int i = 0; i < values.length; i++) {
