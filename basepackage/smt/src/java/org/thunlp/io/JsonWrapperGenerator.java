...
public class JsonWrapperGenerator {
	...
	public static void main(String[] args) throws Exception {
		...
		while ((l = r.readLine()) != null) {
			...
			} else if (l.startsWith("String:") || l.startsWith("long:") || l.startsWith("double:")
					|| l.startsWith("int")) {
				String[] cols = l.split(":");
		...
		for (Entry<String, String> e : fields.entrySet()) {
			String defaultValue = Character.isUpperCase(e.getValue().charAt(0)) ? "\"\"" : "0";
			out("\tpublic static int JSON_" + e.getKey() + " = " + n + ";");
			out("\tprivate " + e.getValue() + " " + e.getKey() + " = " + defaultValue + ";");
			n++;
		}
		...
		for (Entry<String, String> e : fields.entrySet()) {
			out("\t// " + e.getKey());
			out("\tpublic " + e.getValue() + " get" + initialUpper(e.getKey()) + " () { ");
			out("\t\treturn " + e.getKey() + ";");
			out("\t}");

			out("\tpublic void set" + initialUpper(e.getKey()) + " ( " + e.getValue() + " value ) { ");
		...
		for (Entry<String, String> e : fields.entrySet()) {
			out("\t\tthis." + e.getKey() + " = json.get" + initialUpper(e.getValue()) + "( JSON_" + e.getKey() + ");");
		}
