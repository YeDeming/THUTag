...
public class CRFWordSegment implements WordSegment {
	...
	private boolean initializeCRFPP(String libpath, String model) {
		...
		if (crfpp == null) {
			...
			try {
				...
				cmdWriter = new BufferedWriter(new OutputStreamWriter(cmdStream, defaultCharset));
				resultReader = new BufferedReader(new InputStreamReader(is, defaultCharset));
				done = true;
			} catch (IOException e) {
				crfpp = null;
	...
	public void feedInputPattern(String text, int from, int to) throws IOException {
	...
	private void segmentChinese(String text, int from, int to, List<String> words) {
		// Make input pattern.
		try {
